...
@section('main-content')
    <div class="card card-default">
        <div class="card-header card-header-border-bottom d-flex justify-content-between">
            ...
            <a href="{{route('admin.tips.create')}}" class="btn btn-outline-primary btn-sm text-uppercase">
                <i class="fa fa-plus"></i> Add Tips
            </a>
        </div>
        <div class="card-body">
                ...
                <table id="hoverable-data-table" class="table table-hover nowrap" style="width:100%">
                    ...
                    @foreach($tips as $tip)
                        <tr>
                            ...
                            <td>
                                @if($tip->created_by === null)
                                    {{__('Null')}}
                                @else
                                    @foreach($admins as $admin)
                                        @if($tip->created_by == $admin->id)
                                            {{$admin->name}}
                                        @endif
                                    @endforeach
                                @endif
                            ...
                            <td>
                                @if($tip->updated_by === null)
                                    {{__('Null')}}
                                @else
                                    @foreach($admins as $admin)
                                        @if($tip->updated_by == $admin->id)
                                            {{$admin->name}}
                                        @endif
                                    @endforeach
                                @endif
                            ...
                            <td>
                                <a data-toggle="tooltip" title="Edit" href="{{route('admin.tips.edit',['id'=>$tip->id])}}" class="btn btn-sm btn-primary">
                                    <i class="fa fa-edit"></i>
                                </a>
                                <a  data-toggle="tooltip" title="Trash" href="{{route('admin.tips.delete',['id'=>$tip->id])}}" class="btn btn-sm btn-danger delete">
