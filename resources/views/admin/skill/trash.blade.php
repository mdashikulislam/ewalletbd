                ...
                <table id="hoverable-data-table" class="table table-hover nowrap" style="width:100%">
                    ...
                    @foreach($skills as $skill)
                        <tr>
                            ...
                            <td>
                                @if($skill->deleted_by === null)
                                    {{__('Null')}}
                                @else
                                    @foreach($admins as $admin)
                                        @if($skill->deleted_by == $admin->id)
                                            {{$admin->name}}
                                        @endif
                                    @endforeach
                                @endif
                            ...
                            <td>
                                <a data-toggle="tooltip" title="Restore" href="{{route('admin.skill.trash.restore',['id'=>$skill->id])}}" class="btn btn-sm btn-primary restore">
                                    <i class="fa fa-recycle"></i>
                                </a>
                                <a  data-toggle="tooltip" title="Delete" href="{{route('admin.skill.trash.delete',['id'=>$skill->id])}}" class="btn btn-sm btn-danger delete">
