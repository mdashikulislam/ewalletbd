                ...
                <table id="hoverable-data-table" class="table table-hover nowrap" style="width:100%">
                    ...
                    @foreach($partners as $partner)
                        <tr>
                            ...
                            <td>
                                @if($partner->deleted_by === null)
                                    {{__('Null')}}
                                @else
                                    @foreach($admins as $admin)
                                        @if($partner->deleted_by == $admin->id)
                                            {{$admin->name}}
                                        @endif
                                    @endforeach
                                @endif
                            ...
                            <td>
                                <a data-toggle="tooltip" title="Restore" href="{{route('admin.partner.trash.restore',['id'=>$partner->id])}}" class="btn btn-sm btn-primary restore">
                                    <i class="fa fa-recycle"></i>
                                </a>
                                <a  data-toggle="tooltip" title="Delete" href="{{route('admin.partner.trash.delete',['id'=>$partner->id])}}" class="btn btn-sm btn-danger delete">
